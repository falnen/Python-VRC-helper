...
class Tabi(ttk.Frame):
    ...
    def __init__(self,parent,message_display,data = None, name="New Controller",controller_id=None, destroy=None):
        ...
        def filter_set(_):
            if self.Avatar.get() == 'Any':
                for avatar, data in self.saved_avatars:
                    parameters = data[1]
        ...
        self.Avatar.bind('<<ComboboxSelected>>',filter_set)
        self.Avatarlabel = ttk.Label(Header_frame,text='Assigned avatar:')
        ...
        self.Avatar.grid(row=0,column=1,sticky='n',columnspan=3)
        self.Avatarlabel.grid(row=0,column=0,sticky='n',padx=[100,0])
        self.default_parameters_filter.grid(row=1,column=0,sticky='nse',columnspan=2,pady=[0,5])
        self.learned_parameters_filter.grid(row=1,column=3,sticky='nsw',columnspan=2,pady=[0,5])
    ...
    def Lookup(self,Stick,OSCmessage=None,VRCevent=None):
        ...
        Received_value = self.normalize(OSCmessage)
